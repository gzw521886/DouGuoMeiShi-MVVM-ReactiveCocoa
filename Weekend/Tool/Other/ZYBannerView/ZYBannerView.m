...
@property (nonatomic, strong) myLayout *flowLayout;
...
- (void)scroolToLeft{
    ...
    [UIView animateWithDuration:0.8 animations:^{
        self.collectionView.contentOffset=CGPointMake((self.itemCount-1)*self.width, 0);
    }];
...
- (UICollectionView *)collectionView
{
    if (!_collectionView) {
        ...
        _collectionView.tag=noDisableHorizontalScrollTag;
       ...
       // [_collectionView flashScrollIndicators];
        _collectionView.scrollIndicatorInsets=UIEdgeInsetsMake(0, -2, -3, -2);
...
{
    if (!_flowLayout) {
        _flowLayout = [[myLayout alloc] init];
